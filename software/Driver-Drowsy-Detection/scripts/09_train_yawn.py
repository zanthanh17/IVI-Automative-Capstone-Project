#!/usr/bin/env python3
"""Script 09: Train Yawn CNN (MobileNetV2 alpha=0.35)."""
...
def train_model(task, model, base, data_dir, model_out_dir):
    ...
    def load_ds(split):
        ...
        # No .cache() to avoid OOM on large dataset (~42k images)
    ...
    def callbacks(phase):
        return [
            ...
            tf.keras.callbacks.TensorBoard(
                log_dir=f"logs/{task}_p{phase}_{ts}", histogram_freq=0
            ),
        ]
    ...
    base.trainable = True
    unfreeze = cfg["models"]["yawn_cnn"]["unfreeze_layers"]
...
if __name__ == "__main__":
    print("=== Training Yawn CNN ===")
    model, base = build_yawn_cnn(64)
    model.summary()
    train_model("yawn", model, base, "data/splits/mouth", "models/yawn_cnn")
